    ...
    def run(self, attempt: int) -> RunResult:
        ...
        while True:
            ...
            if finished:
                ...
                log.info(
                    "[%s] Proba %d: ZAKONCZONO status=%s czas=%.1fs",
                    self.name,
                    attempt,
                    status,
                    duration,
            ...
            log.info(
                "[%s] Proba %d: status=%s (kolejne sprawdzenie za %ds)...",
                self.name,
                attempt,
                status,
                self.poll_interval,
...
class GitHubRunner(PipelineRunner):
    ...
    def trigger(self) -> RunHandle:
        ...
        response = self._request(
            "POST", dispatch_url, json={"ref": self.cfg.github_ref}
        )
...
class GitLabRunner(PipelineRunner):
    ...
    def poll(self, handle: RunHandle) -> tuple[bool, str, Optional[float]]:
        ...
        if duration is None:
            started = _parse_iso(data.get("started_at")) or _parse_iso(
                data.get("created_at")
            )
    ...
    def _await_build_start(self, queue_url: str) -> str:
        ...
        for _ in range(60):
            ...
            if data.get("cancelled"):
                raise TriggerError(
                    f"[{self.name}] Zadanie zostalo anulowane w kolejce."
                )
...
class MetricsWriter:
    ...
    def write(self, result: RunResult) -> None:
        duration = (
            "" if result.duration_seconds is None else f"{result.duration_seconds:.2f}"
        )
        self._writer.writerow(
            [result.platform, result.attempt, result.status, duration]
        )
...
def run_benchmark(cfg: Config) -> None:
    ...
    log.info("=" * 70)
    log.info(
        "Start eksperymentu: %d iteracji x %d platformy.", cfg.iterations, len(runners)
    )
        ...
        for attempt in range(1, cfg.iterations + 1):
            ...
            for runner in runners:
                try:
                    ...
                    Exception
                ) as exc:  # noqa: BLE001 — pojedynczy blad nie moze przerwac badan
    ...
    log.info("=" * 70)
    log.info(
        "Eksperyment zakonczony w %.1f min. Wyniki zapisano: %s",
        elapsed / 60.0,
        output_path,
    )
    log.info("=" * 70)
...
def main() -> int:
    ...
    try:
        ...
        log.warning(
            "Przerwano przez uzytkownika (Ctrl+C). Czesciowe wyniki sa zapisane."
        )
        return 130
    except PipelineError as exc:
